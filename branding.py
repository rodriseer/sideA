...
# Shown in window title, About, first-run dialog, etc.
APP_NAME = "Photo Metadata Assistant"
WINDOW_TITLE = APP_NAME

# Subfolder created next to the folder the user selects (CSV, logs, previews).
# Use a neutral, descriptive name; avoid client or project codenames.
OUTPUTS_SUBFOLDER = "PhotoMetadata_Output"

# Per-user config directory name (LOCALAPPDATA / Application Support / ~/.config).
# Keep stable across releases; ASCII, no spaces. Independent of APP_NAME so you
# can retitle the app without orphaning saved credentials.
...
APP_TRUST_NOTE = (
    "Uses Google Cloud Vision for image analysis. Your Google service account credentials file "
    "stays on this computer; it is only used to sign in to Google’s servers."
)

# Pipeline / completion (CLI + GUI status — accurate about XMP and output files)
...
# Main window — quick pick without opening the full guide
...
LABEL_STATUS = "Activity"
LABEL_VISION_SETUP = "Google Vision Setup"

CREDENTIALS_STATUS_CONFIGURED = "Connected successfully"
...
FIRST_RUN_BTN_SETUP = "Setup Guide"
...
# --- Google Vision Setup dialog (service account JSON only) -------------------
...
CREDENTIALS_HELP_NOTE = (
    "Google Cloud gives you a JSON file for a “service account.” That is what this app uses—"
    "not a browser API key. The file stays on your computer."
)
CREDENTIALS_INTRO = (
    "Quick steps:\n\n"
